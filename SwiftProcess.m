...
if AntennaType==1 %%%JRC or other boom antenna
    ...
    [xgrid, ygrid]=meshgrid(-50:GridCellSize:max(x(:)-50),-400:GridCellSize:400); %trim for short JRC antenna
    elseif GridNode > 1.2 || GridNode < 2.2
    %%%%%%%%%%%%%grid settings for JRC at 800 setting (1/4miNavico) and 2m grid node
    [xgrid, ygrid]=meshgrid(-50:GridCellSize:max(x(:)-50),-600:GridCellSize:600); %trim for short JRC antenna
    elseif GridNode > 2.2 || GridNode < 3.2
    %%%%%%%%%%%%%grid settings for JRC at 1.6km setting (1/2miNavico) and 3m grid node
    [xgrid, ygrid]=meshgrid(-50:GridCellSize:max(x(:)-50),-1400:GridCellSize:1400); %trim for short JRC antenna
    elseif GridNode > 3.2
    %%%%%%%%%%%%%grid settings for JRC at 2.5km setting (3/4miNavico) and 5m grid node
    [xgrid, ygrid]=meshgrid(-50:GridCellSize:max(x(:)-50),-2300:GridCellSize:2300); %trim for short JRC antenna
    else
    end
...
%     [xgrid, ygrid]=meshgrid(min(easting)-50:GridCellSize:max(x(:)-150),min(northing)-200:GridCellSize:max(northing)+200); %trim for short JRC antenna
    ...
    elseif GridNode > 1.2 || GridNode < 5
    %%%%%%%%%%%%%grid settings for JRC at 800 setting (1/4miNavico) and 2m grid node
    [xgrid, ygrid]=meshgrid(-50:GridCellSize:max(x(:)-100),-400:GridCellSize:400); %trim for short JRC antenna
    else
    end
else
end
...
choice=questdlg('Would you like to create Swift bathy products now?',...
                'Yes', 'No');
switch choice
    case 'Yes'
    ...
    case 'No'
        disp([choice '....consider processing currents and hazards next'])
        products=2;
        clear
end
...
switch choice
    ...
    case 'No'
        disp([choice '....Consider waves and hazards next'])
        products=2;
        clear
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%Pre-set Model WAVE INPUT PARAMETERS IN SCRIPT FILE%%%%%
%%%%%%%%%create gui to continue processing or stop%%%%%
choice=questdlg('Do you want to examine wave hazards now?',...
    'Yes', 'No');
switch choice
    case 'Yes'
        disp([choice '....As you wish'])
        prompt='What is the offshore wave height?';
        Hrms1=input(prompt);
        JandBwavemodel;%%%execute product processing script file
    case 'No'
        disp([choice '....Go have a beer then'])
        clear
end
