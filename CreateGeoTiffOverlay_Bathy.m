...
%+is CCW
% RotateBaseTiff=imrotate(tifData,-20);%%Pass Compass Offset in future
...
% load('test');
load([ProcFold 'BathyProducts']);
plot(EastingCenter,NorthingCenter,'r*');
pcolor(rectX,rectY,depthSmooth); shading flat
caxis([-6 0])
contour(rectX,rectY,depthSmooth,'k')
colorbar
...
saveas(gcf,[ProcFold 'BathyChart_Overlay.tif']);
