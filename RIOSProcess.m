function [XGrid, YGrid, IGrid, Time_total, r, c, p]=RIOSProcess;
...
f = msgbox({'Select 1 for boom or 2 for 4G dome Antenna Type';'Suggest 1m grid node for <800m (1/8nm Navico)';'Suggest 2m grid node for 800m range (1/4nm Navico)';'Suggest 3m grid node for 1600m range (1/2nm Navico)';'Suggest 5m grid node for 2500m (3/4nm Navico)';'Compass offset rotate +CW & -CCW'},'Guidance','custom',grdimage);
...
%     plot(easting,northing,'+'); hold on
    ...
    X=repmat(easting,[1 n]);
    Y=repmat(northing,[1 n]);
...
[xgrid, ygrid]=meshgrid(min(x(:)-400):GridCellSize:max(x(:)-400),min(y(:)-400):GridCellSize:max(y(:)-400)); %trim for short JRC antenna

% %%%%%%%%%%%%%%%%%%%%%%%%%%AUTOMATED GRIDDING OPTIONS%%%%%%%%%
% if AntennaType==1 %%%JRC or other boom antenna
%     if GridNode< 1.2
%     % %%%%%%%%%%%%%Range settings for JRC and 4G at <800m setting (1/4miNavico or <) and 1m grid node
...
%     [xgrid, ygrid]=meshgrid(min(x(:)-200):GridCellSize:max(x(:)-200),min(y(:)-200):GridCellSize:max(y(:)-200)); %trim for short JRC antenna
%     else
%     end
% elseif AntennaType==2%%%4G or other dome antenna
%     if GridNode< 1.2
%     % %%%%%%%%%%%%%Range settings for JRC and 4G at <800m setting (1/4miNavico or <) and 1m grid node
%     [xgrid, ygrid]=meshgrid(min(easting)-50:GridCellSize:max(x(:)-150),min(northing)-150:GridCellSize:max(northing)+150); %trim for short JRC antenna
%     elseif GridNode > 1.2 | GridNode < 2.2
%     %%%%%%%%%%%%%grid settings for JRC at 800 setting (1/4miNavico) and 2m grid node
%     [xgrid, ygrid]=meshgrid(min(easting)-50:GridCellSize:max(x(:)-100),min(northing)-250:GridCellSize:max(northing)+250); %trim for short JRC antenna
%     elseif GridNode > 2.2 | GridNode < 3.2
%     %%%%%%%%%%%%%grid settings for JRC at 1.6km setting (1/2miNavico) and 3m grid node
...
save([ProcFold 'ProcessedFile'],'IGrid', 'XGrid', 'YGrid', 'Time_total', 'r', 'c', 'p', 'GridNode', 'TideLevel','WetDryZ');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
